...
arrived 09:22:40Z; (6) `setProject({skill:'huntAnimals',args:{anyMob:true,radius:32,repeat:true}})`
at 09:22:54.290Z, correctly overriding a decider `chopTrees` dispatch that landed on the same episode
moments earlier (`by:'decider'`, refused anyway on the same food gate) — failed twice more,
identical `no cow within 32 blocks`. **Same discrepancy run #2 already found and never got fixed**:
