...
- **Decider latency fix** (2fea6d8): driver grace keys on an explicit `DRIVEN=1` spawn meta flag (5th field) instead of the
  OWNER label; an unparsed-reply retry rides the next 20s poll instead of the 120s per-bot gap; `bench/decider-latency-replay.js`
  drives the real daemon logic (12/12).
