...
## Producing — acquire by MAKING, not just withdrawing (producer.js v2)

The produce-side of self-sufficiency: on a fresh/depot-less world it MAKES a consumable from
raw materials the bot can mine and craft, so a driverless bot isn't stuck when the depot can't
supply. It's the fallback the agenda's RESTOCK rung calls after a failed withdraw. Ships in
TWO shapes:

- **`produce` SKILL** — the shape the agenda / a driver uses (queued, task-tracked, cancellable):
  ```sh
  ./task.sh <port> start produce '{"resource":"torch","count":16}'
  # task.result -> {ok:true, made:16, how:'crafted', reason:...}   (mines coal + chops wood->planks->sticks->craft)
  ```
- **`__skills.produce(bot, resource, count, opts)` METHOD** — the implementation, for calling
  from INSIDE another skill's fn (pass `{ctx}` so it cancels at a step boundary). Don't await it
  from an agenda act directly — that holds the loop for the whole run; use the skill.
...
(chop → craft). Result `{ok, made, how:'mined'|'crafted'|'gathered', reason}`. ONE-SHOT (the
ladder owns retry), returns PARTIAL success (`made` may be < `count`) rather than throwing — a
typed `reason` (`no_pickaxe`, `no_coal_nearby`, `no_wood`, `no_space`, `partial`, `unproduceable`)
says why it stopped. Self-bootstraps the pickaxe (crafts a wooden one from gathered wood if none),
never chases wood/ore down a ravine, and cancels cleanly if survival stops it. Does NOT produce
FOOD — bread is farmCycle's/cooking's domain.
